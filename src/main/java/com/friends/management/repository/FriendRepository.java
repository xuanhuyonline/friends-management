...
public interface FriendRepository extends JpaRepository<Friend, Long> {
    ...
    @Query("select f from Friend f where f.user.id = :userId1 and f.friend.id = :userId2")
    Friend friendStatus(Long userId1, Long userId2);

    @Query("select count(f) > 0 from Friend f where f.user.id = :userId1 and f.friend.id = :userId2 and f.subscriber = true")
    boolean receivedUpdate(Long userId1, Long userId2);

    @Query("select count(f) > 0 from Friend  f where (f.user.id = :userId1 and f.friend.id = :userId2 and f.status = 'BLOCK') or (f.friend.id = :userId1 and f.user.id = :userId2 and f.status = 'BLOCK')")
